...
async function runHat(id,opt={}){
  ...
  try{
    ...
    stopElapsed();state.meta.currentHat=null;state.meta.phase=null;const m=state.resultMeta[id],attempts=e.attempts||[],last=attempts.at(-1)||null;
    m.status=e.name==="AbortError"?"interrupted":"invalid";m.phase=null;m.lastError=errorData(e);m.validation={...emptyValidation(),errors:e.details||[e.message]};
    m.generation={...m.generation,attempts,attemptCount:attempts.length,structuredAttempted:attempts.length>0,startedAt:attempts[0]?.startedAt||m.generation.startedAt,completedAt:now(),durationMs:attempts.reduce((sum,x)=>sum+(x.durationMs||0),0),promptLength:last?.promptLength||0,responseLength:last?.responseLength||0,contextUsageBefore:last?.measuredContextUsage??last?.contextUsageBefore??null,contextWindow:last?.contextWindow??null};m.updatedAt=now();
    state.meta.status=e.name==="AbortError"?"interrupted":"paused";state.meta.interruptedHat=id;
    if(state.runSummary){state.runSummary.status=e.name==="AbortError"?"interrupted":"failed";state.runSummary.completedAt=now();}
    if(previous){state.results[id]=previous;state.meta.interruptedHat=null;state.meta.status="paused";state.resultMeta[id]={...previousMeta,lastError:errorData(e),status:"warning",validation:{...previousMeta.validation,warnings:[...normalizeWarnings(previousMeta.validation?.warnings,id),createWarning("REGENERATION_FAILED","review_required",`再生成に失敗したため、以前の結果を保持しました: ${e.message}`,id)]},lastFailedGeneration:m.generation};}
    debugLog("hat-run-failed",{hatId:id,error:errorData(e),attempts});await queueAutosave("hat-failed");renderAll();return {ok:false,continue:false};
